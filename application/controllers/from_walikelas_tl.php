...
class From_walikelas_tl extends CI_Controller {
	...
	public function daftar_siswa()
		{	
			if($this->session->userdata('nama_login')){
				...
				$data['confirm']	 =	$this->db->get('tbl_confirmasi');
				//$data['data_wali']		 =	$this->db->get_where('tbl_walikelas',$nama_wali);
				$data['content'] 	 = 	'view_siswatl';
				$this->load->view('view_admin',$data);
				}else{
					$this->login();
				}
	...
	public function input_nilai($nik)
	{
		if($this->session->userdata('nama_login')){
			...
			$data['content'] 	 = 	'view_input_nilaitl';
			$this->load->view('view_admin',$data);
			}else{
				$this->login();
			}
